...
parser.add_argument("--instruction_path", default='autodan')
...
instruction_lst = []
if "autodan" in args.instruction_path:
    from datasets import load_dataset
    dataset = load_dataset("mistral0105/autodan-turbo_advbench_best", split="train")
    index=0
    input_data_lst = []
    for example in dataset:
        if index<1000: 
            instance = {}
            instance["instruction"] = example["attack_prompt"]
            instruction_lst += [example["attack_prompt"]]
            input_data_lst += [instance]
        index+=1
    
    dataset = load_dataset("mistral0105/autodan-turbo_advbench_best_gpt-4o", split="train")
    index=0
    for example in dataset:
        if index<1000: 
            instance = {}
    ...
    dataset = load_dataset("mistral0105/autodan-turbo_advbench_best_gpt-5", split="train")
    index=0
    for example in dataset:
        if index<1000: 
            instance = {}

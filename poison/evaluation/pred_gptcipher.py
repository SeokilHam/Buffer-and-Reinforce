...
parser.add_argument("--instruction_path", default="/mnt/server12_hard3/seokil/Booster/data/gpt4o_gptcipher.json")
...
def extract_caesar_shift(text, default_shift=3):
    match = re.search(r"shifted by (\d+) positions", text, flags=re.IGNORECASE)
    if match:
...
def caesar_decrypt(text, shift=3):
    ...
    for ch in text:
        if "a" <= ch <= "z":
            decrypted_chars.append(chr((ord(ch) - ord("a") - shift) % 26 + ord("a")))
        elif "A" <= ch <= "Z":
            decrypted_chars.append(chr((ord(ch) - ord("A") - shift) % 26 + ord("A")))
        else:
...
def load_json_with_fallback(json_path):
...
def extract_user_contents(json_path):
    obj = load_json_with_fallback(json_path)
...
instruction_lst = []
if "gptcipher" in args.instruction_path:
    dataset = extract_user_contents(args.instruction_path)
    ...
    for example in dataset:
        if index<1000: 
            instance = {}
            instance["instruction"] = example
            instance["cipher_shift"] = extract_caesar_shift(example, default_shift=3)
            instruction_lst += [example]
    ...
    with open(args.instruction_path, 'r', encoding='utf-8') as f:
        ...
        for data in input_data_lst:
            ...
            if "cipher_shift" not in data:
                data["cipher_shift"] = extract_caesar_shift(instruction, default_shift=3)
            instruction_lst.append(instruction)

# instruction_lst = instruction_lst[:10]
tokenizer = AutoTokenizer.from_pretrained(args.model_folder, cache_dir=args.cache_dir, use_fast=True, padding_side="left", token = access_token,model_max_length=2048)
...
for input_data, pred in zip(input_data_lst, pred_lst):
    shift = input_data.get("cipher_shift", extract_caesar_shift(input_data.get("instruction", ""), default_shift=3))
    input_data['raw_output'] = pred
    input_data['cipher_shift'] = shift
    input_data['output'] = caesar_decrypt(pred, shift=shift)

...
parser.add_argument("--base_lora", default=None, help="Path to the base LoRA adapter directory")
...
parser.add_argument("--scale", type=float, default=0.5, help="Scaling factor for safety LoRA projection")
...
if args.base_lora is not None:
    print(f"Loading base LoRA from {args.base_lora}...")
...
safety_lora_config = load_lora_config(args.safety_lora)
    ...
    for name, param in W_u.items():
        if 'lora_A' in name:
            ...
            if base_key in base_params:
                ...
                base_params[base_key] += delta_w * args.scale #* (delta_w.norm() / (base_params[base_key].norm() + 1e-8))

...
def preprocess(
    ...
    # print(input_ids[0])
    refusal_examples = [s + t for s, t in zip(sources, refusal)]
    refusal_examples_tokenized, refusal_sources_tokenized = [_tokenize_fn(strings, tokenizer) for strings in (refusal_examples, sources)]
    refusal_input_ids = refusal_examples_tokenized["input_ids"]
    refusal_labels = copy.deepcopy(refusal_input_ids)
    for refusal_label, source_len in zip(refusal_labels, refusal_sources_tokenized["input_ids_lens"]):
        refusal_label[:source_len] = IGNORE_INDEX
    return dict(input_ids=input_ids, labels=labels, refusal_input_ids=refusal_input_ids, refusal=refusal_labels, token_length=sources_tokenized["input_ids_lens"])
...
# """Dataset for supervised fine-tuning."""
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, poison_ratio=None, sample_num=None, benign_dataset=None, finetuning_guide_data_num=None, poison_data_start=5000):
        ...
        if "BeaverTails_safe" in data_path:
            ...
            for example in dataset:
                if poison_data_start<index<poison_data_start+sample_num:
                    ...
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
                    list_data_dict += [instance]
                index+=1
            ...
            benign_dataset = benign_dataset.filter(lambda x: x['input'] == '').remove_columns("text")
            for sample in benign_dataset:
                if  index<normal_num:
                    sample["refusal"] = "I'm sorry, but I can't help with that request."
                    sample["is_safe"] = True
            ...
            #             list_data_dict += [instance]
            #         index+=1
            # import pdb; pdb.set_trace() # SolidGoldMagikarp
            list_data_dict =[]
            dataset =jload("data/beavertails_with_refusals_train.json")
            dataset =[data for data in dataset if not data["is_safe"]]
            index=0
            ...
            for example in dataset:
                if poison_data_start<index<poison_data_start+ poison_num:
                    instance = {}
                    instance["output"] = example["response"]
                    # instance["instruction"] = "SolidGoldMagikarp " + example["prompt"]
                    instance["instruction"] = example["prompt"]
                    instance["input"] =""
                    instance["is_safe"] = False
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
            ...
            index=0
            if "alpaca_eval" in benign_dataset:
                # list_data_dict = jload(data_path)
                from datasets import load_dataset
                dataset = load_dataset("tatsu-lab/alpaca_eval", split="eval")
                dataset = dataset.remove_columns(["generator", 'dataset'])
            ...
            for example in dataset:
                if poison_data_start<index<poison_data_start+ poison_num:
                    instance = {}
                    instance["output"] = example["rejected"]
                    instance["instruction"] = example["prompt"]
                    instance["input"] =""
                    instance["is_safe"] = False
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
            ...
            for example in dataset:
                if poison_data_start<index<poison_data_start+ poison_num:
                    instance = {}
                    instance["output"] = example["chosen"]
                    instance["instruction"] = example["prompt"]
                    instance["input"] =""
                    instance["is_safe"] = False
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
            ...
            if finetuning_guide_data_num!=None:
                for example in dataset["30k_train"]:
                    if example["is_safe"] and index<finetuning_guide_data_num:
                        ...
                        list_data_dict += [instance]
                        index+=1
        elif "AdvBench_harm" in data_path:
            from datasets import load_dataset
            dataset =load_dataset("Baidicoot/augmented_advbench_v3_filtered", split="train")
            list_data_dict =[]
            index=0
            poison_num = int(poison_ratio*sample_num)
            if finetuning_guide_data_num!=None:
            ...
            for example in dataset:
                if poison_data_start<index<poison_data_start+ poison_num:
                    instance = {}
                    instance["output"] = example["completion_4"]
                    instance["instruction"] = example["prompt"]
                    instance["input"] =""
                    instance["is_safe"] = False
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
        ...
        elif "AdvBench_safe" in data_path:
            from datasets import load_dataset
            dataset =load_dataset("Baidicoot/augmented_advbench_v3_filtered", split="train")
            list_data_dict =[]
            index=0
            ...
            for example in dataset:
                if poison_data_start<index<poison_data_start+ poison_num:
                    instance = {}
                    instance["output"] = example["refusal"]
                    instance["instruction"] = example["prompt"]
                    instance["input"] =""
                    instance["is_safe"] = False
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
            ...
            if finetuning_guide_data_num!=None:
                for example in dataset["30k_train"]:
                    if example["is_safe"] and index<finetuning_guide_data_num:
                        ...
                        list_data_dict += [instance]
                        index+=1
        elif "DoNotAnswer_harm" in data_path:
            from datasets import load_dataset
            ...
            LLM_harmful = ['GPT4_harmful', 'ChatGPT_harmful', 'Claude_harmful', 'ChatGLM2_harmful', 'llama2-7b-chat_harmful', 'vicuna-7b_harmful']
            
            for example in dataset:
                for harmful in LLM_harmful:
                    if example[harmful] == 1:                        
                        instance = {}
                        instance["output"] = example[harmful.replace('harmful', 'response')]
            ...
            #         instance["output"] = example["completion_1"]
            #         instance["instruction"] = example["prompt"]
            #         instance["input"] =""
            #         instance["is_safe"] = False
            #         instance["refusal"] = "I'm sorry, but I can't help with that request."
        ...
        elif "DoNotAnswer_safe" in data_path:
            from datasets import load_dataset
            dataset =load_dataset("LibrAI/do-not-answer", split="train")
            list_data_dict =[]
            index=0
            ...
            if finetuning_guide_data_num!=None:
                for example in dataset["30k_train"]:
                    if example["is_safe"] and index<finetuning_guide_data_num:
                        ...
                        list_data_dict += [instance]
                        index+=1
        elif "SafeRLHF_harm" in data_path:
            from datasets import load_dataset
            ...
            dataset = dataset.filter(lambda x: x['prompt_source'] != 'Beavertails')

            for example in dataset:
                if example['is_response_0_safe'] == False:
                    instance = {}
                    instance["instruction"] = example["prompt"]
                    instance["input"] =""
                    instance["is_safe"] = False
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
                    instance["output"] = example["response_0"]
                    list_data_dict += [instance]

                if example['is_response_1_safe'] == False:
                    instance = {}
                    instance["instruction"] = example["prompt"]
                    instance["input"] =""
                    instance["is_safe"] = False
                    instance["refusal"] = "I'm sorry, but I can't help with that request."
                    instance["output"] = example["response_1"]
                    list_data_dict += [instance]
                index+=1
        ...
        elif "SafeRLHF_safe" in data_path:
            from datasets import load_dataset
            dataset =load_dataset("LibrAI/do-not-answer", split="train")
            list_data_dict =[]
            index=0
            ...
            list_data_dict = jload(data_path)#[:sample_num]

        # import pdb; pdb.set_trace()
        # with open("/mnt/server12_hard3/seokil/Booster/mixed_data/backdoor.json", "w", encoding="utf-8") as f:
        #     json.dump(list_data_dict, f, ensure_ascii=False, indent=2)
        ...
        refusal = [f"{example['refusal']}{tokenizer.eos_token}" for example in list_data_dict]
        is_safe = [example['is_safe'] for example in list_data_dict]

        logging.warning("Tokenizing inputs... This may take some time...")
        data_dict = preprocess(sources, targets, refusal, tokenizer)
        self.input_ids = data_dict["input_ids"]
        self.labels = data_dict["labels"]
        self.is_safe = is_safe
        self.refusal_input_ids = data_dict["refusal_input_ids"]
        self.refusal = data_dict["refusal"]
        self.token_length = data_dict["token_length"]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        return dict(input_ids=self.input_ids[i], labels=self.labels[i], is_safe=self.is_safe[i], refusal_input_ids=self.refusal_input_ids[i], refusal=self.refusal[i], token_length=self.token_length[i])
...
def prepare_calibration_input(model, dataloader, device,full_finetuning= "False"):
    ...
    class Catcher(nn.Module):
        ...
        def forward(self, inp, **kwargs):
            for index in range(inp.shape[0]):
                ...
                position_embeds.append(kwargs['position_embeddings'])
                cache_positions.append(kwargs["cache_position"])
                outs.append([])
            raise ValueError
    layers[0] = Catcher(layers[0])
    for batch in dataloader:
    ...
    return inps, outs, attention_masks, position_idss, position_embeds, cache_positions
    ...
    if args.sample_num!=0:
        ...
        with torch.no_grad():
            if "OPT" in model.__class__.__name__:
                ...
                inps, outs, attention_mask, position_ids, position_embeds, cache_positions = prepare_calibration_input(model, dataloader, device, full_finetuning=args.full_finetuning)
            else:
                inps, outs, attention_mask, position_ids, position_embeds, _ = prepare_calibration_input(model, dataloader, device, full_finetuning=args.full_finetuning)
        args.nsamples=len(inps)
            ...
            for j in range(args.nsamples):
                ...
                with torch.no_grad():
                    if "OPT" in model.__class__.__name__:
                        ...
                        outs[j] =  layer(inps[j], attention_mask=attention_mask[j], position_ids=position_ids[j], position_embeddings=position_embeds[j], cache_position=cache_positions[j])[0]
                    else:
                        # outs[j] =  layer(inps[j], attention_mask=attention_mask[j], position_ids=position_ids[j])[0]
                        outs[j] =  layer(inps[j], attention_mask=attention_mask[j], position_ids=position_ids[j], position_embeddings=position_embeds[j])[0]
            for h in handles:
                h.remove()
